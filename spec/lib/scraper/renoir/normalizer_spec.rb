    ...
    context "when passing an array of non clean movies" do
      ...
      it "returns the clean movies" do
        expect(normalizer.normalize(input)).to match([ {
          ...
          language: :vo,
          duration: 101,
          showtimes: [ { date: Time.utc(Date.today.year, Date.today.month, Date.today.day, 15, 50) } ]
        },
        {
          poster: "https://subdomain.domain.com/imagenes/hash.jpg",
          ...
          language: :vose,
          duration: 123,
          showtimes: [ { date: Time.utc(Date.today.year, Date.today.month, Date.today.day, 16, 0) }, { date: Time.utc(Date.today.year, Date.today.month, Date.today.day, 18, 0) }, { date: Time.utc(Date.today.year, Date.today.month, Date.today.day, 22, 45) } ]
        } ])
      end
    end
